// ==========================================
// BÍBLIA DA IGREJA
// SISTEMA DE SELEÇÃO DE CAPÍTULOS
// ==========================================


// Quantidade de capítulos de cada livro
const capitulosBiblia = {

    "Gênesis": 50,
    "Êxodo": 40,
    "Levítico": 27,
    "Números": 36,
    "Deuteronômio": 34,
    "Josué": 24,
    "Juízes": 21,
    "Rute": 4,
    "1 Samuel": 31,
    "2 Samuel": 24,
    "1 Reis": 22,
    "2 Reis": 25,
    "1 Crônicas": 29,
    "2 Crônicas": 36,
    "Esdras": 10,
    "Neemias": 13,
    "Ester": 10,
    "Jó": 42,
    "Salmos": 150,
    "Provérbios": 31,
    "Eclesiastes": 12,
    "Cânticos": 8,
    "Isaías": 66,
    "Jeremias": 52,
    "Lamentações": 5,
    "Ezequiel": 48,
    "Daniel": 12,
    "Oséias": 14,
    "Joel": 3,
    "Amós": 9,
    "Obadias": 1,
    "Jonas": 4,
    "Miquéias": 7,
    "Naum": 3,
    "Habacuque": 3,
    "Sofonias": 3,
    "Ageu": 2,
    "Zacarias": 14,
    "Malaquias": 4,

    "Mateus": 28,
    "Marcos": 16,
    "Lucas": 24,
    "João": 21,
    "Atos": 28,
    "Romanos": 16,
    "1 Coríntios": 16,
    "2 Coríntios": 13,
    "Gálatas": 6,
    "Efésios": 6,
    "Filipenses": 4,
    "Colossenses": 4,
    "1 Tessalonicenses": 5,
    "2 Tessalonicenses": 3,
    "1 Timóteo": 6,
    "2 Timóteo": 4,
    "Tito": 3,
    "Filemom": 1,
    "Hebreus": 13,
    "Tiago": 5,
    "1 Pedro": 5,
    "2 Pedro": 3,
    "1 João": 5,
    "2 João": 1,
    "3 João": 1,
    "Judas": 1,
    "Apocalipse": 22
};


// ==========================================
// ELEMENTOS DA TELA
// ==========================================

const livro = document.getElementById("livro");

const capitulo = document.getElementById("capitulo");

const versiculo = document.getElementById("versiculo");

const btnMostrar = document.getElementById("btnMostrar");

const referencia = document.getElementById("referencia");

const textoVersiculo = document.getElementById("textoVersiculo");


// ==========================================
// QUANDO ESCOLHER UM LIVRO
// ==========================================

livro.addEventListener("change", function () {

    const livroSelecionado = livro.value;

    // Limpa os capítulos antigos
    capitulo.innerHTML =
        '<option value="">Selecione o capítulo</option>';

    // Limpa os versículos
    versiculo.innerHTML =
        '<option value="">Selecione o versículo</option>';

    // Se nenhum livro foi escolhido
    if (!livroSelecionado) {
        return;
    }

    const quantidadeCapitulos =
        capitulosBiblia[livroSelecionado];


    // Cria os capítulos
    for (
        let numero = 1;
        numero <= quantidadeCapitulos;
        numero++
    ) {

        const opcao =
            document.createElement("option");

        opcao.value = numero;

        opcao.textContent =
            "Capítulo " + numero;

        capitulo.appendChild(opcao);
    }

});


// ==========================================
// QUANDO ESCOLHER UM CAPÍTULO
// ==========================================

capitulo.addEventListener("change", function () {

    const livroSelecionado = livro.value;

    const capituloSelecionado =
        capitulo.value;


    // Limpa os versículos
    versiculo.innerHTML =
        '<option value="">Selecione o versículo</option>';


    if (
        !livroSelecionado ||
        !capituloSelecionado
    ) {
        return;
    }


    /*
    Por enquanto vamos colocar 50 versículos
    como exemplo.

    Depois vamos substituir isso pela
    quantidade REAL de cada capítulo.
    */

    for (let numero = 1; numero <= 50; numero++) {

        const opcao =
            document.createElement("option");

        opcao.value = numero;

        opcao.textContent =
            "Versículo " + numero;

        versiculo.appendChild(opcao);
    }

});


// ==========================================
// BOTÃO MOSTRAR
// ==========================================

btnMostrar.addEventListener("click", function () {

    const livroSelecionado = livro.value;

    const capituloSelecionado =
        capitulo.value;

    const versiculoSelecionado =
        versiculo.value;


    if (
        !livroSelecionado ||
        !capituloSelecionado ||
        !versiculoSelecionado
    ) {

        alert(
            "Selecione o livro, capítulo e versículo."
        );

        return;
    }


    // Mostra a referência
    referencia.textContent =
        `${livroSelecionado} ${capituloSelecionado}:${versiculoSelecionado}`;


    // Por enquanto
    textoVersiculo.textContent =
        "O texto do versículo será carregado aqui.";

});
